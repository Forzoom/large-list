...
在ios开发中有被称为UITableView的组件，在android开发中有被称为ListView的组件，它们通过销毁不可见区域的元素，来达到性能优化的目的，我们在组件当中也采用这样的逻辑，即便列表中有10000个元素，当屏幕可视区域中可能只有5个元素，那么我们只显示5个元素，这将大大减少我们的网页对于硬件资源的消耗。
...
先来考虑最简单的情况，假设所有的\<PostCard\>的高度都是100，我们来实现上面所说的效果。list数组依然包含所有的数据，但我们需要另外一个数组，决定需要展示哪些数据
...
```
现在我们要想办法确定startIndex和endIndex，startIndex是可视列表(displayList)中的第一个元素的下标，endIndex是最后一个元素的下标+1，在固定高度的情况下，startIndex和endIndex的计算十分简单
```javascript
...
```
到此为止，我们已经完成了一个最简单的逻辑，但是还没有完，我们还需要对元素的样式进行一些适当的补充
```html
...
为了让组件更加通用一些，PostCard的高度可能在不同的情况下显示的高度不同，甚至在浏览的过程中，PostCard都可能在发生变化，LargeList应该做好其下元素的高度会发生变化的准备。当元素的高度发生变化时，我们应该做什么？元素高度发生变化，其他元素的位置可能会发生变化，我们只更新其他可视元素的top数据
...
当高度可变的时候，startIndex和endIndex就不再像固定高度时，可以那么轻松地计算出来，我们需要在整个list中寻找我们需要显示的列表内容，二分查找是个不错的选择。
...
我们之前将\<PostCard\>这样的组件直接写入到\<LargeList\>组件中，但是我们希望将LargeList应用于各种各样的场景下，所以现在我们通过插槽将PostCard传入LargeList中
